...
int main(void){
    ...
    Texture2D tex = LoadTexture("assets/gray.png");

    ParticleSystem fire;

    initialise_system(&fire, tex, 40.0f);
    ...
    while (!WindowShouldClose()){
        ...
        for (int i = 0; i < 8; i++){
            emit_particles(&fire, (Vector2){640, 360});
        }

        update_particles(&fire, dt);
        
        BeginDrawing();
        ClearBackground(BLACK);

        render_particles(&fire, BLEND_ADDITIVE);
